...
awk -v sap_sid="$SAP_SID" -v sap_instance_num="$SAP_INSTANCE_NUM" -v sap_host_name="$SAP_HOST_NAME" '{gsub("<SAP_SID>", sap_sid); gsub("<SAP_HOST_NAME>", sap_host_name); gsub("<SAP_INSTANCE_NUM>", sap_instance_num);}1' sid_config_template.txt > ${SAP_SID}_configfile
temp=`awk -v sap_adm_pw="$SAP_ADM_PW" -v sid_adm_pw="$SID_ADM_PW" -v system_pw="$SYSTEM_PW" '{gsub("<SAP_ADM_PW>", sap_adm_pw); gsub("<SID_ADM_PW>", sid_adm_pw); gsub("<SYSTEM_PW>", system_pw);}1' sid_passwords_template.txt`
